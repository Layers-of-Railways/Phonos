...
public class HeadsetModel extends BasicModel {
    ...
    public void render(LivingEntity entity, MatrixStack matrices, VertexConsumerProvider buffers, ModelTransform transform, int light, int overlay, ItemStack stack, HeadsetItem item) {
        matrices.push();
        this.root.setTransform(transform);
        var texture = item.getTexture(stack);

        this.microphone.visible = entity instanceof IMicrophoneHoldingClientPlayerEntity player && player.phonos$getHoldingState() == WIRELESS;
